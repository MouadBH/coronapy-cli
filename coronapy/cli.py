...
def init():
    ...
    f = Figlet(font="slant")
    click.echo(
        str(color.prCyan(f.renderText("Corona CLI")))
        + str(color.prGreen("Corona-cli"))
        + str(color.prYellow("v1.3.9"))
        + " by MouadBH."
    )
    click.echo(str(color.prPurple("Follow me on Github: https://github.com/MouadBH")))
    click.echo(
        str(color.prPurple("Follow me on Twitch: https://www.twitch.tv/chibaku_"))
    )
    print(" \n\n")
...
@click.option(
    ...
    "-s",
    default="cases",
    help="Data of each continent sorted by the parameter.",
)
@click.option(
    "--limit", "-l", default=0, help="Limit the number of the returned results."
)
...
@click.option(
    ...
    "-s",
    default="cases",
    help="Data of each country sorted by the parameter.",
)
@click.option(
    "--limit", "-l", default=0, help="Limit the number of the returned results."
)
...
@click.option(
    "--hist", "-h", is_flag=True, help="Draw a histogram shows data in last 20 day."
)
...
def country(country, chart, hist, type):
    ...
    if hist:
        ...
        with yaspin(text="Drawing a histogram of " + country, color="cyan") as sp:
            try:
                labels_hist, hist_data = get_country.get_country_hist(
                    country_full_name, type
                )
        ...
        click.secho(
            "COVID-19 '" + type + "' for last 20 day in " + country_full_name + ".",
            bg="black",
            fg="yellow",
            blink=True,
            bold=True,
        ...
        try:
            ...
            click.secho(
                "No historical data found in " + country + ".",
                bg="black",
                fg="red",
                bold=True,
            )
